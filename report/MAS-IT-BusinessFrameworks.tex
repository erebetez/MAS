...
\section{BPEL}
The BPEL (Business Process Execution Language) was an answer to the process problem that came from the it world. It is very technical and the process description is made with ifs and loops that are also known in common programming languages. 
...
This chapter shall give an overview how the infrastructure that supports the web applicatin and the development of it, has to look. 
...
So it runs an Jee environment like jBoss or tomcat.
The operating system is therefore not so relevant. 

To keep things simple linux was choosen for the example server.
